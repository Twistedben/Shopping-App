...
// We add in this._items to ensure we don't lose the previous list of products
  ...
  Future<void> fetchAndSetProducts([bool filterByUser = false]) async {
    ...
    try {
      ...
      final fetchedData = json.decode(response.body) as Map<String,
          dynamic>; // Says we have a Map coming in where the keys are strings and values are dynamic (The values are also Maps but declaring that would give us a error)
      if (fetchedData == null) {
      ...
      fetchedData.forEach((prodId, prodData) {
        // We need convert the data into product objects based on our Product class, since we need product from items
      ...
      _items =
          loadedProducts; // Assigns the above empty NONFINAL variable with the new loadedProducts array/list
  ...
  Future<void> addProduct(Product product) async {
    ...
    try {
      ...
      final response =
          await http // Await wraps all the code that comes after it, allowing then() and catchError() to be removed. Works like synchronous code
  ...
  Future<void> deleteProduct(String id) async {
    ...
    var existingProduct = _items[
        existingProductIndex]; // Uses the index to find the deleting product.
    _items.removeAt(
        existingProductIndex); // Removes the deleting item from the list, but not in memory
    notifyListeners();
    ...
    if (response.statusCode >= 400) {
      _items.insert(existingProductIndex,
          existingProduct); // If for some reason removal fails, then we can readd that deleted item back into the list.
      notifyListeners();
      throw HttpException('Could not delete product.');
    }
    existingProduct =
        null; // It was deleted in firebase, so delete it in memory, too.
